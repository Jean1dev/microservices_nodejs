...
class SendMessage {
    ...
    public async send (): Promise<void> {
        ...
        await delay(PACE_TIMING)
        console.log('2. Click conversation with title matching target. Ignore message results.')
        await clickConversarion(this.page, this.target).catch((error) => { throw error })
        await delay(PACE_TIMING)
        console.log('Clears conversations search bar.')
        await pressClearSearch(this.page).catch((error) => { throw error })
        await delay(PACE_TIMING)
        console.log('4. Inject the message into the text field. Error if text field not found.')
        await injectMessage(this.page, this.message).catch((error) => { throw error })
        await delay(PACE_TIMING)
        console.log('5. Press button to send message. Error if button not found.')
